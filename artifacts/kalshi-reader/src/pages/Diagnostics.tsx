...
type CandidateHistory = { orders: CandidateOrder[] };
...
type BoundaryReport = { available: boolean; timelines: BoundaryTimeline[] };
...
async function getJson<T>(path: string): Promise<T> {
  ...
  if (!response.ok) throw new Error((body as { error?: string } | null)?.error ?? `${response.status} ${response.statusText}`);
  if (body == null) throw new Error('Empty diagnostics response');
  return body;
}
...
export default function Diagnostics() {
  ...
  const [backFlipError, setBackFlipError] = useState<string | null>(null);
  const [lastLoadedAt, setLastLoadedAt] = useState<number | null>(null);

  const load = async () => {
    const [h, b, f] = await Promise.allSettled([
      ...
      getJson<BackFlipReport>('/api/diagnostics/back-flips'),
    ]);

    if (h.status === 'fulfilled') setHistory(h.value);
    if (b.status === 'fulfilled') setBoundary(b.value);
    if (h.status === 'rejected' || b.status === 'rejected') {
      setError(h.status === 'rejected' ? h.reason?.message ?? 'Candidate history unavailable' : b.status === 'rejected' ? b.reason?.message ?? 'Boundary audit unavailable' : null);
    } else {
      setError(null);
    }

    if (f.status === 'fulfilled') {
      setBackFlips(f.value);
      setBackFlipError(null);
    } else {
      setBackFlipError(f.reason?.message ?? 'Back Flip evidence unavailable');
    }
  ...
  const rows = useMemo(() => {
    ...
    return orders.map((order) => {
      ...
      return {
        ...
        reservationMs,
        submissionMs: offset(submission, openTimeMs),
  ...
  const todayBackFlips = useMemo(() => {
    const today = easternDate(Date.now());
    return [...(backFlips?.rows ?? [])]
      .filter((row) => Number.isFinite(row.armedAtMs) && easternDate(row.armedAtMs) === today)
  ...
  const medianReservation = reservations.length
    ...
    : null;
  const armedBackFlips = todayBackFlips.filter((row) => row.status === 'armed').length;
  const consumedBackFlips = todayBackFlips.length - armedBackFlips;
  ...
  return <div className="min-h-[100dvh] bg-background text-foreground">
    <div className="mx-auto max-w-7xl min-h-[100dvh] border-x border-border">
      ...
      <main className="p-4 sm:p-6 space-y-5">
        ...
        {backFlipError && <div className="border border-amber-500/40 bg-amber-500/5 p-3 text-sm text-amber-700 dark:text-amber-300">Back Flip table unavailable: {backFlipError}</div>}
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="border border-border bg-card p-4"><div className="font-mono text-[10px] uppercase text-muted-foreground">Back Flip arms today</div><div className="mt-2 font-mono text-2xl">{todayBackFlips.length}</div><div className="mt-1 text-xs text-muted-foreground">{armedBackFlips} still armed · {consumedBackFlips} no longer armed</div></div>
          <div className="border border-border bg-card p-4"><div className="font-mono text-[10px] uppercase text-muted-foreground">Zero-fill timing split</div><div className="mt-2 font-mono text-sm">{fastZeroFills} fast · {lateZeroFills} late</div><div className="mt-1 text-xs text-muted-foreground">Median {formatOffset(medianReservation)}</div></div>
        </section>

        <section className="border border-border bg-card">
          <div className="border-b border-border p-4 flex flex-wrap justify-between gap-2">
            <div><h2 className="font-semibold">Back Flip arm ledger</h2><p className="mt-1 text-xs text-muted-foreground">Direct SELECT-only read from the durable override table. An A-window authoritative zero fill should arm exactly the next 15-minute B window.</p></div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] font-mono text-xs">
              <thead className="bg-muted/30 text-[10px] uppercase text-muted-foreground"><tr><th className="p-3 text-left">Armed ET</th><th className="p-3 text-left">A ticker</th><th className="p-3 text-left">Missed side</th><th className="p-3 text-left">B target ET</th><th className="p-3 text-left">Status</th></tr></thead>
              <tbody>
                {todayBackFlips.map((row) => <tr key={row.sourceCandidateOrderId} className="border-t border-border">
                  <td className="p-3 whitespace-nowrap">{etTime(row.armedAtMs)}</td>
                  <td className="p-3 whitespace-nowrap">{row.sourceTicker}</td>
                  <td className="p-3 font-semibold">{row.missedSide.toUpperCase()}</td>
                  <td className="p-3 whitespace-nowrap">{etTime(row.targetOpenTimeMs)}</td>
                  <td className={row.status === 'armed' ? 'p-3 font-semibold text-amber-600' : 'p-3 font-semibold text-emerald-600'}>{row.status.toUpperCase()}</td>
                </tr>)}
                {todayBackFlips.length === 0 && <tr><td colSpan={5} className="p-6 text-center text-muted-foreground">No Back Flip arms found for today.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
        ...
        <section className="border border-border bg-card">
          <div className="border-b border-border p-4"><h2 className="font-semibold">Today's order timing</h2><p className="mt-1 text-xs text-muted-foreground">Offsets are measured from the exchange-provided market open time. This panel reads the existing append-only boundary audit ledger; it does not affect trading.</p></div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1100px] font-mono text-xs">
              <thead className="bg-muted/30 text-[10px] uppercase text-muted-foreground"><tr><th className="p-3 text-left">Open ET</th><th className="p-3 text-left">Ticker</th><th className="p-3 text-left">Fill</th><th className="p-3 text-left">Timing</th><th className="p-3 text-right">Probe</th><th className="p-3 text-right">Active</th><th className="p-3 text-right">Usable</th><th className="p-3 text-right">Evaluate</th><th className="p-3 text-right">Reserve</th><th className="p-3 text-right">Submit</th></tr></thead>
              <tbody>
                {rows.map((row) => <tr key={row.order.id} className={row.zeroFill ? 'border-t border-border bg-amber-500/5' : 'border-t border-border'}>
                  <td className="p-3 whitespace-nowrap">{etTime(row.openTimeMs)}</td><td className="p-3 whitespace-nowrap">{row.order.ticker}</td><td className={row.zeroFill ? 'p-3 font-semibold text-amber-600' : 'p-3'}>{row.fillLabel}</td><td className={`p-3 font-semibold ${row.timing.className}`}>{row.evidenceMissing ? 'NO AUDIT' : row.timing.label}</td><td className="p-3 text-right">{formatOffset(row.probeMs)}</td><td className="p-3 text-right">{formatOffset(row.activeMs)}</td><td className="p-3 text-right">{formatOffset(row.usableMs)}</td><td className="p-3 text-right">{formatOffset(row.evaluationMs)}</td><td className="p-3 text-right font-semibold">{formatOffset(row.reservationMs)}</td><td className="p-3 text-right">{formatOffset(row.submissionMs)}</td>
                </tr>)}
        ...
        <section className="border border-border bg-muted/20 p-4 text-sm">
          <div className="font-semibold">What we are verifying</div>
          <p className="mt-2 text-muted-foreground">For every authoritative zero fill in Window A, we expect a durable Back Flip arm targeting Window B exactly 15 minutes later. If zero fills are present but arms are absent, or old target rows remain armed after B has passed, that points to a runtime synchronization problem rather than ordinary 50¢ liquidity.</p>
        </section>
      </main>
    </div>
  </div>;
