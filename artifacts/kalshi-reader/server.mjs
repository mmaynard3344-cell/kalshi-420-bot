...
async function serviceLedgerTodayDiagnostics(req, res) {
  ...
  try {
    const data = await withReadOnlyDb(async (client) => {
      ...
      const big = await client.query(`
        SELECT b.strategy, COUNT(*)::int AS n,
               COALESCE(SUM(
                 CASE WHEN lower(mr.result)=lower(b.side)
        ...
        JOIN market_results mr ON mr.ticker=b.ticker
        WHERE to_char(to_timestamp(b.created_at_ms/1000.0) AT TIME ZONE 'America/New_York','YYYY-MM-DD')=$1
...
async function recentMarketResultsDiagnostics(req, res) {
  ...
  try {
    const rows = await withReadOnlyDb(async (client) => {
      const result = await client.query(`
        SELECT ticker, lower(result) AS result, resolved_at_ms
        FROM market_results
        WHERE ticker LIKE 'KXETH15M-%'
          AND lower(result) IN ('yes','no')
          AND COALESCE(resolved_at_ms,0) >= $1
        ORDER BY resolved_at_ms DESC NULLS LAST
        LIMIT 2000
      `, [Date.now() - 7 * 86_400_000]);
      return result.rows;
    });
    ...
    console.error('Recent market-results read failed', error);
    return send(res, 500, JSON.stringify({ error:'Recent market results unavailable', rows:[] }), 'application/json; charset=utf-8');
  }
}
...
async function shadowPerformanceDiagnostics(req, res) {
  ...
  try {
    const data = await withReadOnlyDb(async (client) => {
      ...
      const [a2IntentExists, lIntentExists, evalExists] = await Promise.all([
      ...
      const serviceHealth = (service, intents) => {
        ...
        return {
          ...
          settledIntentsRecent: recentIntents.filter((row) => row.settlementResult === 'yes' || row.settlementResult === 'no').length,
        };
      };
...
server.listen(port, '0.0.0.0', () => {
  ...
  setTimeout(async()=>{try{
    ...
    const replay=(name,ladder)=>{
      let day='',side='no',step=0,cum=0,peak=0,maxDD=0,maxStake=0,wins=0,losses=0,step6=0,maxLossStreak=0,lossStreak=0;
      const daily=new Map();
      for(const m of uniq){
        const d=dayOf(m.openMs); if(d!==day){day=d;side='no';step=0;}
        const stake=ladder[step]; const contracts=Math.max(1,Math.round(stake/50)); // cents at 50c
        const won=side===m.result; const pnl=won?stake:-stake;
        cum+=pnl; peak=Math.max(peak,cum); maxDD=Math.max(maxDD,peak-cum); maxStake=Math.max(maxStake,stake);
        if(step===5)step6++;
        if(won){wins++; lossStreak=0; side=side==='yes'?'no':'yes'; step=0;}
        else {losses++; lossStreak++; maxLossStreak=Math.max(maxLossStreak,lossStreak); step=step>=5?0:step+1;}
        const rec=daily.get(d)??{pnl:0,trades:0};rec.pnl+=pnl;rec.trades++;daily.set(d,rec);
      }
      const profitableDays=[...daily.values()].filter(x=>x.pnl>0).length;
      return {name,ladder,markets:uniq.length,wins,losses,winRate:uniq.length?wins/uniq.length:null,totalPnlCents:cum,maxDrawdownCents:maxDD,maxStakeCents:maxStake,step6Uses:step6,maxLossStreak,days:daily.size,profitableDays,profitableDayRate:daily.size?profitableDays/daily.size:null};
    };
    console.log('FULL_ARCHIVE_LADDER_REPLAY '+JSON.stringify({
      first:uniq[0]??null,last:uniq[uniq.length-1]??null,count:uniq.length,
      current:replay('current',[50,50,50,100,250,300]),
      old:replay('pre_scale',[1500,3000,6000,12000,24000,32000])
    }));
  }catch(e){console.error('FULL_ARCHIVE_LADDER_REPLAY_FAILED',String(e?.message??e))}},1500);
  void Promise.all([
