(() => {
  ...
  const routeOf = (o, bfWindows, bfAvailable) => {
    const c=clientId(o);
    ...
    if(c.endsWith(':eth420-live-v1')){if(!bfAvailable)return'420 Special'; const t=orderTime(o); return bfWindows.some(w=>t>=w&&t<w+900000)?'420 · Back Flip':'420 · Jump';}
    return'Other ETH';
  };
  ...
  function renderOrders(orders,fills,market,bf,bfAvailable){
    ...
    if($('currentOrderRows'))$('currentOrderRows').innerHTML=currentRows.length?currentRows.map(o=>{const route=routeOf(o,bfWindows,bfAvailable),actual=actualPrincipal(o,fills),principal=actual??maxPrincipal(o),price=orderPrice(o);return '<div class="current-order-row"><div><div class="current-order-route">'+esc(route)+' · '+esc(sideOf(o))+'</div><div class="sub">'+esc(statusOf(o))+' · '+esc(fullTime(orderTime(o)))+'</div></div><div><div class="eyebrow">Price / principal</div><div class="v">'+(price==null?'—':Math.round(price)+'¢')+' · '+moneyDollars(principal)+'</div></div><div><div class="eyebrow">Requested</div><div class="v">'+esc(reqOf(o)??'—')+'</div></div><div><div class="eyebrow">Filled / remaining</div><div class="v">'+fillOf(o)+' / '+esc(remOf(o)??'—')+'</div></div></div>'}).join(''):'';
