...
if (source.includes('P&L since Aug 29')) process.exit(0);
...
const pnlSection = `        <section className="border border-border bg-card">
          <div className="p-4 sm:p-5 border-b border-border flex flex-wrap items-start justify-between gap-3">
            <div><div className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">P&L since Aug 29</div><h2 className="mt-1 font-semibold">Actual Kalshi ETH fills</h2><p className="mt-1 text-xs text-muted-foreground">All executed KXETH15M fills · actual fill prices and exchange fees · Eastern Time</p></div>
          ...
          {actualPnl.coverageLimited && <div className="border-t border-border bg-amber-500/5 px-4 py-2 text-xs text-amber-700 dark:text-amber-300">The 1,000-fill exchange limit was reached before Aug 29. Totals shown may be incomplete.</div>}
