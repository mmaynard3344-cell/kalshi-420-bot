...
async function buildAshley() {
  const commitSha = resolveCommitSha();
  const distDir = path.resolve(artifactDir, "dist");
  await rm(distDir, { recursive: true, force: true });
  await esbuild({
    entryPoints: [path.resolve(artifactDir, "src/ashley.ts")],
    platform: "node",
    bundle: true,
    format: "esm",
    outdir: distDir,
    outExtension: { ".js": ".mjs" },
    logLevel: "info",
    define: { "process.env.COMMIT_SHA": JSON.stringify(commitSha) },
    external: [
...
buildAshley().catch((err) => { console.error(err); process.exit(1); });
