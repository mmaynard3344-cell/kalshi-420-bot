/**
 * Build and run the fillReconciler unit tests with esbuild-plugin-pino so
...
const outdir  = "/tmp/ts-fillreconciler";
const outfile = path.join(outdir, "fillReconciler.test.mjs");

const result = await build({
  entryPoints: [path.join(root, "src/lib/fillReconciler.test.ts")],
  platform:    "node",
  bundle:      true,
  format:      "esm",
  outdir,
