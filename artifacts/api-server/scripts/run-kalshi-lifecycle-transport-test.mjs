...
const outdir = "/tmp/kalshi-lifecycle-transport";
const outfile = path.join(outdir, "kalshiLifecycleTransport.test.mjs");
await build({
  entryPoints: [path.join(root, "src/lib/kalshiLifecycleTransport.test.ts")],
...
const result = spawnSync(process.execPath, ["--test", outfile], { cwd: outdir, stdio: "inherit" });
