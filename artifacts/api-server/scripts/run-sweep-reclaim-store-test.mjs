...
const outdir = "/tmp/ts-sweep-reclaim-store";
const outfile = path.join(outdir, "tradeStore.sweepReclaim.test.mjs");
await build({
  entryPoints: [path.join(root, "src/lib/tradeStore.sweepReclaim.test.ts")],
  platform: "node",
  bundle: true,
  format: "esm",
  outdir,
