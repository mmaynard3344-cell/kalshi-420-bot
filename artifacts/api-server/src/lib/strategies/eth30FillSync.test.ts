...
import {
  planCanonicalFillLedger, settlementReadiness, buildAuthoritativeFillChunks, CHUNK_EVIDENCE_NOTE,
  type AuthoritativeFillChunk, type Eth30OwnedOrderRef,
} from "./eth30FillSync.js";
import { computeEth30Report } from "./eth30Report.js";
import type { Eth30PositionEventParams, Eth30TickerClaim, Eth30StrategyOrder } from "../tradeStore.js";
import type { KalshiFillWire } from "../kalshiFillNormalizer.js";

const TICKER = "KXETH15M-26AUG161200-15";
const entryRef: Eth30OwnedOrderRef = {
  id: `entry:${TICKER}`, ticker: TICKER, easternDate: "2026-08-16", role: "entry", filledContracts: 8,
};
const exitRef: Eth30OwnedOrderRef = {
  id: `exit:${TICKER}:1`, ticker: TICKER, easternDate: "2026-08-16", role: "exit", filledContracts: 6,
};
function chunk(fillId: string, contracts: number, priceCents: number, at: number, feeCents: number | null = null): AuthoritativeFillChunk {
  return { fillId, contracts, fillPriceCents: priceCents, feeCents, occurredAtMs: at };
}
function legacyEvent(id: string, orderId: string, delta: number, after: number, price: number): Eth30PositionEventParams {
  return {
...
const existingFeeNull: Eth30PositionEventParams = {
  ...
  settlementResult: null, note: CHUNK_EVIDENCE_NOTE, occurredAtMs: 100,
};
...
test("multi-level IOC entry records each chunk at its actual execution price", () => {
  ...
  const plan = planCanonicalFillLedger([entryRef], [], chunks);
  ...
  assert.equal(plan.appends[0].note, CHUNK_EVIDENCE_NOTE);
  // Replay with the canonical events already present is a no-op.
  const replay = planCanonicalFillLedger([entryRef], plan.appends, chunks);
  assert.deepEqual([replay.deleteIds, replay.appends], [[], []]);
});

test("legacy approximate event at the wrong price is replaced wholesale, never blended", () => {
  ...
  const chunks = new Map([[entryRef.id, [chunk("f1", 5, 28, 100), chunk("f2", 3, 30, 101)]]]);
  const plan = planCanonicalFillLedger([entryRef], [legacy], chunks);
...
test("legacy total falling inside a chunk boundary cannot overcount", () => {
...
test("incomplete chunk coverage defers the whole ticker — nothing is written", () => {
  // Exit order reports 6 filled but the fills endpoint only evidenced 4.
  const plan = planCanonicalFillLedger(
    [entryRef, exitRef],
    [],
    new Map([
  ...
  const failed = planCanonicalFillLedger([entryRef], [], new Map([[entryRef.id, null]]));
  assert.deepEqual(failed.deferredOrderIds, [entryRef.id]);
});

test("settlement is gated on chunk evidence only — legacy events never satisfy readiness", () => {
  const chunks = new Map([[entryRef.id, [chunk("f1", 5, 28, 100), chunk("f2", 3, 30, 101)]]]);
  const entryEvents = planCanonicalFillLedger([entryRef], [], chunks).appends;
  const orders = [
  ...
  const before = settlementReadiness(orders, entryEvents);
  ...
  const withLegacy = settlementReadiness(orders, [...entryEvents, legacyExit]);
  ...
  const fullLedger = planCanonicalFillLedger([entryRef, exitRef], [], allChunks).appends;
  const after = settlementReadiness(orders, fullLedger);
  assert.equal(after.ready, true);
  assert.equal(after.openContracts, 2); // 8 entry - 6 exit = 2 remain
  assert.equal(after.hasLegacyEvents, false);
});

test("end to end: canonical chunks + gated settlement produce exact strategy-only P&L", () => {
  ...
  const ledger = planCanonicalFillLedger([entryRef, exitRef], [], allChunks).appends;
  const settlement: Eth30PositionEventParams = {
  ...
  const claim: Eth30TickerClaim = { ticker: TICKER, easternDate: "2026-08-16", claimedAtMs: 1, entryClientOrderId: "c" };
  const entryOrder: Eth30StrategyOrder = {
  ...
  assert.equal(row.cashFlowPnlCents, 300 + 200 - 230); // 270
  assert.equal(row.realizedPnlCents, null, "canonical fill prices alone are insufficient without fee evidence");
  assert.equal(row.status, "settled");
});

test("fee backfill: chunk with fee updates existing null-fee event, settlement is not touched", () => {
  ...
  const settlementEvent: Eth30PositionEventParams = {
    id: `${TICKER}:settlement`, ticker: TICKER, easternDate: "2026-08-16",
  ...
  const existingWithFee: Eth30PositionEventParams = { ...existingFeeNull, feeCents: 3 };
  const replay = planCanonicalFillLedger(
    [{ ...entryRef, filledContracts: 5 }],
...
// ── Fee path tests (production buildAuthoritativeFillChunks) ──────────────────
// These tests exercise the production function shared between
// fetchOwnedFillChunks (HTTP path) and reconciliation — a bug in feeCents
// assignment is caught here rather than mirrored in the test helper.

test("fee dollar-to-cents conversion produces exact cents with no off-by-100 in the production path", () => {
  // 0.0175 dollars = 1.75¢ → rounds to 2¢.  An off-by-100 bug in the
  // production formula would produce 175¢ (÷100 forgotten) or 0¢ (×100 dropped).
  const wireWithFee: KalshiFillWire = { ...BASE_FILL, fee_cost_dollars: "0.0175" };
  const chunks = buildAuthoritativeFillChunks([wireWithFee], "yes");
...
test("fee_cost_dollars='0' is accepted by the production path and contributes feeCents=0", () => {
  // A zero-fee fill is valid (e.g. maker fills on Kalshi). Rejecting it would
  // cause the chunk count to fall short of the order fill total, permanently
  // deferring settlement.
  const wireZeroFee: KalshiFillWire = { ...BASE_FILL, fee_cost_dollars: "0" };
  const chunks = buildAuthoritativeFillChunks([wireZeroFee], "yes");
...
test("fill missing fee_cost_dollars is dropped by the production path so reconciliation defers", () => {
  // An incomplete API response without fee data must not be silently accepted
  // with an assumed zero fee — the production path drops the record, which
  // causes the chunk count to fall short of the fill total and forces a defer.
  ...
  const chunks = buildAuthoritativeFillChunks([wireWithoutFee], "yes");
