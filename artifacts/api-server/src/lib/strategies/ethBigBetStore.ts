  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      if (isBkFreshBalanceCapitalPolicyEnabled()) {
        if (evaluateBkFreshBalanceOnly(params.capital.availableBalanceCents, params.requestedRiskCents) !== "allow") return "capital_blocked";
      } else {
    ...
    console.error("ethBigBetStore: reservation transaction failed", {
      strategy: params.intent.strategy,
