...
/**
 * Big-bet exchange adapter with same-client-id recovery.
 *
 * A transport/response ambiguity must not poison the strategy+market reservation
 * for the rest of the 15-minute window. We first prove exchange identity by the
 * immutable client_order_id. If the first POST still cannot be found after a
 * short authenticated-read backoff, we retry the POST exactly once with the
 * SAME client_order_id and reconcile again. This preserves one economic intent
 * while rescuing a request that never reached Kalshi.
 */
export function createEthBigBetKalshiSubmitter(exchangeIndex: number): EthBigBetExchangeSubmitter | null {
  ...
  return {
    async submit(input): Promise<EthBigBetSubmitResult> {
      ...
      // One bounded retry, deliberately reusing the immutable client ID.
