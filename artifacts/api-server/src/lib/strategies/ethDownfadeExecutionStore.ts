...
function positiveInteger(value: unknown): number | null {
  const n = typeof value === "bigint" ? Number(value)
    : typeof value === "number" ? value
    : typeof value === "string" && /^\d+$/.test(value) ? Number(value) : NaN;
  return Number.isSafeInteger(n) && n > 0 ? n : null;
}

async function allUnresolvedRiskCents(tx: DbLike): Promise<number | null> {
  ...
  for (const row of rows) {
    const wager = positiveInteger(row["wager_cents"]);
    const price = positiveInteger(row["limit_price_cents"]);
    if (wager == null || price == null || price > 99) return null;
    const risk = ethBigBetCapitalRiskCents(wager, price);
    if (risk < 1 || !Number.isSafeInteger(total + risk)) return null;
    total += risk;
  }
  return total;
}
...
export const ethDownfadeExecutionStore: EthBigBetExecutionStore = {
  async listUnresolvedEthBigBetOrderIds(strategy) {
    if (!DOWNFADE_STRATEGIES.has(strategy)) return [];
    const db = await getDb();
  ...
  async reserveEthBigBetOrder(input): Promise<EthBigBetExecutionReservationResult> {
        ...
        const result = await tx.execute(sql`
          INSERT INTO eth_big_bet_orders
            (id, strategy, order_tag, ticker, market_open_time_ms, side,
             wager_cents, limit_price_cents, requested_contracts, status, created_at_ms, updated_at_ms)
          VALUES
            (${input.orderId}, ${input.intent.strategy}, ${input.intent.orderTag}, ${input.intent.ticker},
             ${input.intent.marketOpenTimeMs}, ${input.intent.side}, ${input.intent.wagerCents},
             ${input.intent.limitPriceCents}, ${input.requestedContracts}, 'reserved',
             ${input.reservedAtMs}, ${input.reservedAtMs})
  ...
  async acknowledgeEthBigBetOrder(input) {
    ...
    if (input.status === "submitted") {
      if (!input.exchangeOrderId) return false;
      const result = await db.execute(sql`
        UPDATE eth_big_bet_orders
        SET kalshi_order_id=${input.exchangeOrderId}, status='submitted', updated_at_ms=${input.acknowledgedAtMs}
        WHERE id=${input.orderId} AND status='reserved' AND kalshi_order_id IS NULL
        RETURNING id
      `);
    ...
    if (input.status === "rejected") {
      const result = await db.execute(sql`
        UPDATE eth_big_bet_orders
        SET status='rejected', filled_contracts=0, realized_pnl_cents=0, updated_at_ms=${input.acknowledgedAtMs}
        WHERE id=${input.orderId} AND status='reserved'
        RETURNING id
      `);
      return ((result as { rows?: unknown[] }).rows ?? []).length === 1;
    }
    const result = await db.execute(sql`
      UPDATE eth_big_bet_orders
      SET status='submission_unknown', updated_at_ms=${input.acknowledgedAtMs}
      WHERE id=${input.orderId} AND status='reserved'
      RETURNING id
    `);
    return ((result as { rows?: unknown[] }).rows ?? []).length === 1;
