/**
 * Pure planning logic for the canonical SOL_30_50 fill-event ledger.
 *
...
// ── SOL_30_50 position event parameter type ────────────────────────────────────
// Mirrors Eth30PositionEventParams but references SOL storage columns.
// Defined here (and re-exported from sol30_50.ts) so that tests and the
// fill-sync module share a single canonical definition without depending on
// tradeStore sol30 APIs that are being added in parallel.
export interface Sol30PositionEventParams {
  id:               string;
  ticker:           string;
  easternDate:      string;
  eventType:        "entry_fill" | "exit_fill" | "settlement";
...
export const SOL30_CHUNK_EVIDENCE_NOTE = "exchange_fill_chunk";

export function isSol30ChunkFillEvent(ev: Sol30PositionEventParams): boolean {
  return (ev.eventType === "entry_fill" || ev.eventType === "exit_fill")
    && ev.note === SOL30_CHUNK_EVIDENCE_NOTE;
}

function isSol30FillEvent(ev: Sol30PositionEventParams): boolean {
  return ev.eventType === "entry_fill" || ev.eventType === "exit_fill";
}

export interface Sol30OwnedOrderRef {
  /** sol30_strategy_orders.id — "entry:{ticker}" or "exit:{ticker}:{seq}". */
  id:          string;
  ticker:      string;
  easternDate: string;
  role:        "entry" | "exit";
...
export interface Sol30AuthoritativeFillChunk {
 ...
 */
export function buildSol30AuthoritativeFillChunks(
  fills: KalshiFillWire[],
  side: "yes" | "no",
  nowMs = Date.now(),
): Sol30AuthoritativeFillChunk[] {
...
export interface Sol30CanonicalLedgerPlan {
  ...
  appends: Sol30PositionEventParams[];
}
 ...
 * no-op.
 */
export function planSol30CanonicalFillLedger(
  orders:         Sol30OwnedOrderRef[],
  existingEvents: Sol30PositionEventParams[],
  chunksByOrder:  Map<string, Sol30AuthoritativeFillChunk[] | null>,
): Sol30CanonicalLedgerPlan {
  const filledOrders = orders.filter((o) => (o.filledContracts ?? 0) > 0);
  const deferred: string[] = [];
  const canonicalInputs: Array<{ order: Sol30OwnedOrderRef; chunk: Sol30AuthoritativeFillChunk }> = [];
  ...
  let running = 0;
  const canonical: Sol30PositionEventParams[] = canonicalInputs.map(({ order, chunk }) => {
    ...
    return {
      ...
      note: SOL30_CHUNK_EVIDENCE_NOTE,
  ...
  const existingChunkById = new Map(
    existingEvents.filter(isSol30ChunkFillEvent).map((ev) => [ev.id, ev]),
  );
  return {
    deferredOrderIds: [],
    deleteIds: existingEvents
      .filter((ev) => isSol30FillEvent(ev) && (!isSol30ChunkFillEvent(ev) || !canonicalIds.has(ev.id)))
      .map((ev) => ev.id),
    ...
    //       in appendSol30PositionEvent to backfill the fee without changing
    //       any other field.
 ...
 * were already sold.
 */
export function sol30SettlementReadiness(
  orders: Array<Pick<Sol30OwnedOrderRef, "id" | "filledContracts">>,
  events: Sol30PositionEventParams[],
): { ready: boolean; openContracts: number; missingOrderIds: string[]; hasLegacyEvents: boolean } {
  const hasLegacyEvents = events.some((ev) => isSol30FillEvent(ev) && !isSol30ChunkFillEvent(ev));
  const missing: string[] = [];
  for (const order of orders) {
    const filled = order.filledContracts ?? 0;
    if (filled <= 0) continue;
    const recorded = events
      .filter((ev) => isSol30ChunkFillEvent(ev) && ev.strategyOrderId === order.id)
