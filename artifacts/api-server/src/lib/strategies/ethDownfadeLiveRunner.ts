...
export async function runEthDownfadeServiceWhenExplicitlyEnabled(input: {
  ...
  const intent = await prepareEthDownfadeServiceIntent({
    role,
    market: input.market,
  ...
  // The capital provider reads this ledger. Initialize it before capital reads;
  // this preserves the B startup-order fix for every frequency service.
  try {
  ...
  const rejectionReason = outcome === "submitted" ? null
    ...
    : outcome === "rejected" ? "exchange_rejected_reason_not_exposed_by_executor"
    : outcome;
