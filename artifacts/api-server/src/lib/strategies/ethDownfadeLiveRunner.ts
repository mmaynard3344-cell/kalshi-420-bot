...
export type EthDownfadeLiveOutcome =
  | "disabled"
  | "no_signal"
  | "capital_unavailable"
  | "capital_blocked"
...
export function isEthDownfadeServiceExecutionPermitted(role = currentEthServiceRole()): role is EthDownfadeRole {
  ...
  return ETH_DOWNFADE_SERVICE_EXECUTION_APPROVED
    && isEthDownfadeRole(role)
    && enablement.valid
    && enablement.mode === "downfade_live_requested"
    && process.env["ETH_DOWNFADE_SERVICE_LIVE_ENABLED"] === "true";
}

export async function runEthDownfadeServiceWhenExplicitlyEnabled(input: {
  market: Eth420CandidateMarket;
  exchangeIndex: number | null | undefined;
}): Promise<EthDownfadeLiveOutcome> {
  ...
  const finish = <T extends EthDownfadeLiveOutcome>(outcome: T, rejectionReason: string | null = null): T => {
    logger.info({ serviceRole: role,
      wagerCents: isEthDownfadeRole(role) ? ETH_DOWNFADE_CONFIG[role].wagerCents : null,
      ticker: input.market.ticker, currentMove, direction, p80, p90, p95, p99,
      validObservationCount, outcome, rejectionReason: rejectionReason ?? signalRejectionReason,
    }, "ETH Downfade evaluation");
    return outcome;
  };

  if (!isEthDownfadeServiceExecutionPermitted(role)) return finish("disabled", "execution_not_permitted");

  const intent = await prepareEthDownfadeServiceIntent({ role, market: input.market,
    onEvaluation: (observation) => {
      currentMove = observation.currentMove; direction = observation.direction;
      p80 = observation.p80; p90 = observation.p90; p95 = observation.p95; p99 = observation.p99;
