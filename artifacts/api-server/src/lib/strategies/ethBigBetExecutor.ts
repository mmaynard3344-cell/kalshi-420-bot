...
export async function submitEthBigBetIntent(input: {
  ...
  const reservation = await input.store.reserveEthBigBetOrder({ orderId, intent: input.intent, requestedContracts: contracts, requestedRiskCents: input.requestedRiskCents, capital: input.capital, reservedAtMs: nowMs });
  ...
  try {
    ...
    if (submitted.kind === "accepted") {
      const acknowledged = await acknowledgeDurably(input.store, { orderId, exchangeOrderId: submitted.exchangeOrderId, status: "submitted", acknowledgedAtMs: Date.now() });
    ...
    if (submitted.kind === "rejected") {
      const acknowledged = await acknowledgeDurably(input.store, { orderId, exchangeOrderId: null, status: "rejected", acknowledgedAtMs: Date.now() });
