...
import { buildEthAshleyIntent, type EthAshleyEvidence } from "./ethAshleySignal.js";
...
export function _setEthAshleyMarketFetcherForTesting(fetcher: MarketFetcher | null): void {
...
export async function prepareEthAshleyIntent(input: {
  market: Eth420CandidateMarket;
  nowMs?: number;
  onEvaluation?: (evidence: EthAshleyEvidence & { rejectionReason: string | null }) => void;
}): Promise<EthBigBetOrderIntent | null> {
  const nowMs = input.nowMs ?? Date.now();
  const empty = (reason: string, partial: Partial<EthAshleyEvidence> = {}) => {
    input.onEvaluation?.({
  ...
  const declineRatio = moveRatio < 0 ? -moveRatio : 0;
  const evidence: EthAshleyEvidence = {
    ticker: input.market.ticker,
  ...
  const intent = buildEthAshleyIntent(evidence);
