...
/**
 * Advances side/step for a zero-fill order using its official market outcome.
 * Since no contracts were filled, this deliberately leaves spent and realized
 * P&L untouched.
 */
 ...
 * actually filled.
 *
 * Old-date guard: if the order's eastern_date does not match the state's
 * current eastern_date, the state table has already rolled over to a new
 * day. In that case we update the order row (idempotent settlement) but do
 * NOT modify the fresh day's sequence or P&L.
 */
  ...
  try {
    return await _db.transaction(async (tx) => {
      ...
      const settled = await tx.execute(sql`
        ...
        RETURNING martingale_step, side, eastern_date, no_price_cents,
