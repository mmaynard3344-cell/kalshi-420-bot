...
export interface SweepReclaimClaimParams {
  id: string; strategyId: string; serviceCode: "L"; displayLabel: string; sourceVenue: string;
  sourceCandleOpenMs: number; sourceCandleCloseMs: number;
  sourceOpen: number; sourceHigh: number; sourceLow: number; sourceClose: number;
  prior24hLow: number; candleRange: number; realBody: number; lowerWick: number; midpoint: number;
  closePositionFraction: number; sweptPrevious24hLow: boolean; wickCondition: boolean; upperHalfClose: boolean;
  qualified: boolean; destinationTicker: string; side: "yes"; claimedAtMs: number;
}
export interface SweepReclaimClaimUpdate {
  id: string; observedYesPriceCents?: number|null; configuredPriceCapCents?: number|null;
  requestedContracts?: number|null; requestedRiskCents?: number|null; correlatedExposureBeforeCents?: number|null;
...
export async function claimSweepReclaimSignal(params: SweepReclaimClaimParams): Promise<boolean> {
  if (!_db || !_healthy) return false;
  try {
    const inserted=await _db.insert(sweepReclaimClaims).values({...params,lifecycleState:"CLAIMED",updatedAtMs:params.claimedAtMs})
      .onConflictDoNothing().returning({id:sweepReclaimClaims.id});
    if(inserted.length===1)_lastWriteMs=Date.now();
    return inserted.length===1;
  } catch(err) {
    _healthy=false; _lastErrorMsg=String(err); _degradedReason=`sweepReclaim.claim failed: ${_lastErrorMsg}`;
    recordDbBlockedOperation("entry"); _scheduleRetry(); logger.error({err,destinationTicker:params.destinationTicker},"sweepReclaim: durable claim failed closed"); return false;
  }
}
export async function updateSweepReclaimClaim(update: SweepReclaimClaimUpdate): Promise<boolean> {
  ...
  for(const [key,value] of Object.entries(update)){if(key!=="id"&&value!==undefined)patch[key]=value;}
  try {
    const rows=await _db.update(sweepReclaimClaims).set(patch as any).where(eq(sweepReclaimClaims.id,update.id)).returning({id:sweepReclaimClaims.id});
...
export async function getSweepReclaimClaim(id:string){
  ...
  catch(err){logger.warn({err,id},"sweepReclaim: claim lookup failed");return null;}
}
export async function findSweepReclaimClaimBySignal(strategyId:string,sourceCandleOpenMs:number,destinationTicker:string){
  if(!_db||!_healthy)return null;
  try{const rows=await _db.select().from(sweepReclaimClaims).where(and(eq(sweepReclaimClaims.strategyId,strategyId),eq(sweepReclaimClaims.sourceCandleOpenMs,sourceCandleOpenMs),eq(sweepReclaimClaims.destinationTicker,destinationTicker))).limit(1);return rows[0]??null;}
  catch(err){logger.warn({err,strategyId,sourceCandleOpenMs,destinationTicker},"sweepReclaim: signal lookup failed");return null;}
}
