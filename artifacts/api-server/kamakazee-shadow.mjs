...
function authHeaders(method, path) {
  ...
  return { "Content-Type": "application/json", "KALSHI-ACCESS-KEY": keyId, "KALSHI-ACCESS-SIGNATURE": signature.toString("base64"), "KALSHI-ACCESS-TIMESTAMP": timestamp };
}

async function authFetch(method, path, body) {
  const response = await fetch(TRADE_BASE + path, { method, headers: authHeaders(method, "/trade-api/v2" + path), body: body === undefined ? undefined : JSON.stringify(body) });
...
async function init() {
  await db.execute(sql`CREATE TABLE IF NOT EXISTS kamakazee_state (singleton INTEGER PRIMARY KEY CHECK (singleton = 1), step INTEGER NOT NULL CHECK (step BETWEEN 0 AND 2), last_signal_ticker TEXT, updated_at_ms BIGINT NOT NULL)`);
  await db.execute(sql`INSERT INTO kamakazee_state(singleton, step, updated_at_ms) VALUES (1, 0, ${Date.now()}) ON CONFLICT (singleton) DO NOTHING`);
  await db.execute(sql`CREATE TABLE IF NOT EXISTS kamakazee_orders (target_ticker TEXT PRIMARY KEY, signal_ticker TEXT NOT NULL, client_order_id TEXT UNIQUE NOT NULL, kalshi_order_id TEXT, step INTEGER NOT NULL, principal_cents INTEGER NOT NULL, requested_contracts INTEGER NOT NULL, filled_contracts NUMERIC, order_status TEXT NOT NULL, official_result TEXT, transition_applied BOOLEAN NOT NULL DEFAULT FALSE, raw_json JSONB, created_at_ms BIGINT NOT NULL, updated_at_ms BIGINT NOT NULL)`);
}
...
async function reconcile() {
  ...
  for (const row of result.rows) {
    ...
    let market;
    try { market = await publicFetch(`/markets/${encodeURIComponent(row.target_ticker)}`); } catch { return false; }
    ...
    if (parsed.filled <= 0) {
      await db.execute(sql`UPDATE kamakazee_orders SET official_result=${official}, transition_applied=TRUE, order_status='zero_fill', updated_at_ms=${Date.now()} WHERE target_ticker=${row.target_ticker}`);
      log("ZERO_FILL", { targetTicker: row.target_ticker, step: row.step, sequenceNeutral: true });
      continue;
    }
    const win = official === "NO";
    const nextStep = 0;
    const transitioned = await db.execute(sql`UPDATE kamakazee_orders SET official_result=${official}, transition_applied=TRUE, updated_at_ms=${Date.now()} WHERE target_ticker=${row.target_ticker} AND transition_applied=FALSE RETURNING target_ticker`);
...
async function evaluate() {
  if (process.env.KAMAKAZEE_LIVE_ENABLED !== "true") { log("FAIL_CLOSED", { reason: "live_disabled" }); return; }
  ...
  const qualifies = efficiency != null && efficiency >= EFFICIENCY_THRESHOLD && previousTwo[0] === "YES" && previousTwo[1] === "YES";
  log("EVALUATION", { latestFinalizedTicker: latest.ticker, previousTwo, directionalEfficiency: efficiency == null ? null : Number(efficiency.toFixed(6)), threshold: EFFICIENCY_THRESHOLD, qualifies });
  ...
  const candidates = (openRaw.markets ?? []).filter((market) => typeof market?.ticker === "string" && market.ticker.startsWith("KXETH15M-") && typeof market?.open_time === "string" && Date.parse(market.open_time) === targetOpenMs);
  if (candidates.length !== 1) { log("FAIL_CLOSED", { reason: "exact_target_market_unavailable", signalTicker: latest.ticker, targetOpenMs }); return; }
  ...
  const principalCents = effectivePrincipalCents();
  const contracts = Math.floor(principalCents / LIMIT_CENTS);
  const clientId = `${targetTicker}:kamakazee-k-v1`;
  const reserved = await db.execute(sql`INSERT INTO kamakazee_orders(target_ticker, signal_ticker, client_order_id, step, principal_cents, requested_contracts, order_status, created_at_ms, updated_at_ms) VALUES(${targetTicker}, ${latest.ticker}, ${clientId}, ${step}, ${principalCents}, ${contracts}, 'reserved', ${Date.now()}, ${Date.now()}) ON CONFLICT (target_ticker) DO NOTHING RETURNING target_ticker`);
  if (reserved.rows.length !== 1) return;

  const payload = { ticker: targetTicker, client_order_id: clientId, side: "ask", count: `${contracts}.00`, price: "0.5000", time_in_force: "good_till_canceled", self_trade_prevention_type: "taker_at_cross", cancel_order_on_pause: true, exchange_index: Number(candidates[0].exchange_index) };
  ...
  try {
    ...
    await db.execute(sql`UPDATE kamakazee_orders SET kalshi_order_id=${parsed.orderId}, filled_contracts=${parsed.filled}, order_status=${parsed.status ?? "submitted"}, raw_json=${JSON.stringify(response)}::jsonb, updated_at_ms=${Date.now()} WHERE target_ticker=${targetTicker}`);
    log("ORDER_SUBMITTED", { targetTicker, side: "NO", principalDollars: principalCents / 100, step, contracts, limitPriceCents: LIMIT_CENTS, timeInForce: "GTC", kalshiOrderId: parsed.orderId });
  } catch (error) {
    log("FAIL_CLOSED", { reason: "submission_failed_or_unknown", targetTicker, error: String(error?.message ?? error), status: error?.status ?? null, body: error?.body ?? null });
  }
}
...
setInterval(() => {
  if (inFlight) return;
  inFlight = true;
  evaluate().catch((error) => log("FAIL_CLOSED", { reason: "unexpected_evaluation_error", error: String(error?.message ?? error) })).finally(() => { inFlight = false; });
}, POLL_MS);
