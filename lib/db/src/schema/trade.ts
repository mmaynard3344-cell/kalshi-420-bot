...
// Additive, versioned storage. The composite uniqueness fence prevents a second
// L order for the same source candle / destination window across workers or restarts.
export const sweepReclaimClaims = pgTable(
  ...
  (table) => [
    uniqueIndex("sweep_reclaim_claim_identity_uq").on(
      table.strategyId,
      table.sourceCandleOpenMs,
      table.destinationTicker,
    ),
