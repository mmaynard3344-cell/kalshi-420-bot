...
const MIN_GAP_MS = 10*60e3, MAX_GAP_MS = 20*60e3;
...
function streakStatsRows(rows){ let max=0,cur=0,prev=null,yesMax=0,noMax=0; for(let i=0;i<rows.length;i++){ const r=rows[i].result; if(i>0){ const dt=rows[i].t-rows[i-1].t; if(dt<MIN_GAP_MS||dt>MAX_GAP_MS){ cur=0; prev=null; } } if(r!=='yes'&&r!=='no'){cur=0;prev=null;continue;} cur=(r===prev)?cur+1:1; prev=r; max=Math.max(max,cur); if(r==='yes') yesMax=Math.max(yesMax,cur); else noMax=Math.max(noMax,cur); } return {max_streak:max,max_yes_streak:yesMax,max_no_streak:noMax}; }
function features(rows){
  if(rows.length<2) return null;
  const floors=rows.map(r=>r.floor);
  const rets=[]; const pairs=[]; let gapPairs=0;
  for(let i=1;i<rows.length;i++){
    const dt=rows[i].t-rows[i-1].t;
    if(dt<MIN_GAP_MS||dt>MAX_GAP_MS){gapPairs++;continue;}
    const ret=(rows[i].floor-rows[i-1].floor)/rows[i-1].floor;
    rets.push(ret); pairs.push({abs:Math.abs(ret),from:rows[i-1],to:rows[i]});
  }
  ...
  const st=streakStatsRows(rows);
  const mp=[...pairs].sort((a,b)=>b.abs-a.abs)[0];
  return {
    n:rows.length,contiguous_pairs:rets.length,gap_pairs:gapPairs,
    ...
    max_move_from:mp?.from?.ticker||null,max_move_to:mp?.to?.ticker||null,max_move_time:mp?.to?.t?iso(mp.to.t):null,
    ...st
  };
}
...
const analogs=histCandidates.sort((a,b)=>a.distance-b.distance).slice(0,5).map(c=>({window_end:iso(c.end),distance:c.distance,features:c.f,next24:futureStats(rows,c.end,24),next48:futureStats(rows,c.end,48)}));
